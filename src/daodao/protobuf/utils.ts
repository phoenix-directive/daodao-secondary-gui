...
import { AnyChain, ChainId, DecodedStargateMsg } from '@/daodao/protobuf/types';
import { StargateMsg, UnifiedCosmosMsg, VoteOption } from '@/daodao/types/contracts';
import {
...
export const cwMsgToProtobuf = (...params: Parameters<typeof cwMsgToEncodeObject>): Any => {
...
export const cwMsgToEncodeObject = (
  ...
  if ('wasm' in msg) {
    ...
    const isSecret = chainId === ChainId.SecretMainnet || chainId === ChainId.SecretTestnet;
    ...
    if ('execute' in wasmMsg) {
      ...
      const encodeObject: EncodeObject = {
        typeUrl:
          'send' in wasmMsg.execute ? SecretMsgExecuteContract.typeUrl : MsgExecuteContract.typeUrl,
        value:
    ...
    if ('update_admin' in wasmMsg) {
      const encodeObject: EncodeObject = {
        typeUrl: isSecret ? SecretMsgUpdateAdmin.typeUrl : MsgUpdateAdmin.typeUrl,
        value: isSecret
          ? ({
              sender,
...
export const getProtobufTypes = (): ReadonlyArray<[string, GeneratedType]> => [
  ...
  // alliance.alliance
  [MsgCreateAllianceProposal.typeUrl, MsgCreateAllianceProposal as GeneratedType],
  [MsgUpdateAllianceProposal.typeUrl, MsgUpdateAllianceProposal as GeneratedType],
  [MsgDeleteAllianceProposal.typeUrl, MsgDeleteAllianceProposal as GeneratedType],
  // gaia.metaprotocols
...
export const makeStargateMessageFromAmino = (msg: AminoMsg): StargateMsg => {
  if (!('type' in msg)) {
    throw new Error('`type` field missing from Amino message, formatted like `cosmos-sdk/...`');
  }
  if (!('value' in msg)) {
