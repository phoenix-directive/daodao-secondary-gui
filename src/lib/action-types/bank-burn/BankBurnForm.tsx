...
import type { BankBurnMsg } from './BankBurnAction';

export function BankBurnForm({
  data,
  onUpdate,
}: {
  data: BankBurnMsg;
  ...
  const burnData = data.bank.burn;

  const currentDenom = burnData.amount?.[0]?.denom || '';
  const currentAmountBase = burnData.amount?.[0]?.amount || '0';
  ...
  const handleDenomChange = (newDenom: string) => {
    ...
    onUpdate(['bank', 'burn', 'amount'], [{ denom: newDenom, amount: baseAmount }]);
  };
  ...
  const handleAmountBlur = () => {
    ...
    const amount = [...(burnData.amount || [{ denom: currentDenom || 'uluna', amount: '0' }])];
    amount[0].amount = baseAmount;
    onUpdate(['bank', 'burn', 'amount'], amount);
  };
