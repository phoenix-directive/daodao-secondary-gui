...
export class Keplr implements ExtensionProviderAdapter {
  ...
  async connect(
    ...
    if (
      this.useExperimentalSuggestChain &&
      this.keplr.experimentalSuggestChain
    ) {
      const defaultCurrency = network.defaultCurrency || DEFAULT_CURRENCY;
      const baseGasPrice = GasPrice.fromString(
      ...
      await this.keplr.experimentalSuggestChain({
        ...
        bech32Config:
          network.bech32Config ||
          defaultBech32Config(network.chainPrefix || DEFAULT_CHAIN_PREFIX),
        currencies: [defaultCurrency, network.stakeCurrency].filter(
          nonNullable
        ),
  ...
  async disconnect(
    ...
    _options?: { network: Network } | undefined
  ): Promise<void> {
    return;
  }
  ...
  async sign(
    ...
    if (wallet.account.isLedger) {
      return await this.signLedger(provider, {
        network,
        wallet,
        messages,
        feeAmount,
    ...
    return await OfflineDirectSigningClient.sign(
      this.keplr.getOfflineSigner(network.chainId),
      {
        network,
        wallet,
        messages,
  ...
  async signLedger(
    ...
    const signResponse = await this.keplr.signAmino(
      network.chainId,
      wallet.account.address,
      signDoc
    );
  ...
  async signAndBroadcast(
    ...
    if (wallet.account.isLedger) {
      // || isInjectiveNetwork(network.chainId)
    ...
    return await SignAndBroadcastClient.execute(
      this.keplr.getOfflineSigner(network.chainId),
      {
        network,
        wallet,
