...
export class CosmosWalletConnect implements MobileProviderAdapter {
  ...
  private accountsCacheKey = 'wc_accounts_cache';
  ...
  private getCachedAccounts(topic: string, chainId: string): CosmosWCAccount[] | null {
    try {
      const cacheStr = localStorage.getItem(this.accountsCacheKey);
      if (!cacheStr) return null;

      const cache = JSON.parse(cacheStr);
      const key = `${topic}:${chainId}`;
      const cached = cache[key];

      if (!cached) return null;

      // Check if cache is expired (60 minutes)
  ...
  private setCachedAccounts(topic: string, chainId: string, accounts: CosmosWCAccount[]): void {
    try {
      const cacheStr = localStorage.getItem(this.accountsCacheKey);
      ...
      localStorage.setItem(this.accountsCacheKey, JSON.stringify(cache));
    } catch {
  ...
  private async getAccounts({
    ...
    const topic = mobileSession.walletConnectSession.topic;
    const chainId = network.chainId;

    // Check cache first
    const cachedAccounts = this.getCachedAccounts(topic, chainId);
    if (cachedAccounts) {
      return cachedAccounts;
    }

    // Fetch from WalletConnect if not cached
    const accounts = (await this.walletConnect.request({
      topic,
    ...
    })) as CosmosWCAccount[] | null;

    // Cache the result
    if (accounts) {
      this.setCachedAccounts(topic, chainId, accounts);
    }
  ...
  async connect(
    provider: WalletMobileProvider,
    {
      network,
      callback,
    }: { network: Network; callback?: ((walletConnection: WalletConnection) => void) | undefined },
  ): Promise<string> {
    if (!this.walletConnect) {
      throw new Error('Wallet Connect is not available');
    }
