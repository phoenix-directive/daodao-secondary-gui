    ...
    if (filters) {
      if (filters.providerId) {
        wallets = wallets.filter(
          (wallet) => wallet.providerId === filters.providerId
        );
      }
      if (filters.chainId) {
        wallets = wallets.filter(
          (wallet) => wallet.network.chainId === filters.chainId
        );
      }
    }
    return wallets;
  ...
  removeWallets: (filters?: { providerId?: string; chainId?: string }) =>
    set((state) => {
      ...
      if (filters.providerId && filters.chainId) {
        wallets = wallets.filter(
          (wallet) =>
            wallet.providerId !== filters.providerId ||
            wallet.network.chainId !== filters.chainId
        );
