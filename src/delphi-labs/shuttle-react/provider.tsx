...
export function ShuttleProvider({
  ...
  const [availableExtensionProviders, setAvailableExtensionProviders] = useState<
    WalletExtensionProvider[]
  >([]);
  ...
  const updateExtensionWallets = useCallback(
  ...
  const checkExtensionProviders = useCallback(() => {
    extensionProviders
      .filter(
        (extensionProvider) => !extensionProvider.initializing && !extensionProvider.initialized,
      )
  ...
  }, [extensionProviders, updateExtensionWallets, withLogging]);
  ...
  const providerInterface = useMemo(() => {
    const mobileConnect = async ({
      ...
      const mobileProvider = availableMobileProviders.find(
    ...
    const disconnect = (filters?: { providerId?: string; chainId?: string }) => {
      internalStore.getWallets(filters).forEach((wallet) => {
        const provider =
          availableExtensionProviders.find(
            (extensionProvider) => extensionProvider.id === wallet.providerId,
          ) ||
          availableMobileProviders.find(
            (mobileProvider) => mobileProvider.id === wallet.providerId,
          );
        if (provider) {
    ...
    const simulate = async ({
      ...
      const provider =
        ...
        availableMobileProviders.find(
          (mobileProvider) => mobileProvider.id === walletToUse.providerId,
        );
    ...
    const verifyArbitrary = async ({
      ...
      return provider.verifyArbitrary({
        wallet: walletToUse,
        data,
        signResult,
      });
    ...
    return {
      ...
      checkExtensionProviders,
    } as ShuttleContextType;
  }, [
    extensionProviders,
    mobileProviders,
    wallets,
  ...
  useEffect(() => {
    if (
      ...
      internalStore.getWallets().length === 0
    ) {
  ...
  useEffect(() => {
    ...
    // interval check if availableExtenionProviders are set
